...
public class GridLayoutManager extends LinearLayoutManager
{
    ...
    private void ensureAnchorIsInFirstSpan(final AnchorInfo anchorInfo) {
        for (int n = this.mSpanSizeLookup.getCachedSpanIndex(anchorInfo.mPosition, this.mSpanCount); n > 0 && anchorInfo.mPosition > 0; n = this.mSpanSizeLookup.getCachedSpanIndex(anchorInfo.mPosition, this.mSpanCount)) {
    ...
    private void measureChildWithDecorationsAndMargin(final View view, final int n, final int n2) {
        ...
        view.measure(this.updateSpecWithExtra(n, layoutParams.leftMargin + this.mDecorInsets.left, layoutParams.rightMargin + this.mDecorInsets.right), this.updateSpecWithExtra(n2, layoutParams.topMargin + this.mDecorInsets.top, layoutParams.bottomMargin + this.mDecorInsets.bottom));
    }
    ...
    View findReferenceChild(int i, final int n, final int n2) {
        ...
        while (i != n) {
            ...
            if (position >= 0) {
                ...
                view4 = view2;
                if (position < n2) {
                    if (this.mSpanSizeLookup.getCachedSpanIndex(position, this.mSpanCount) != 0) {
                        view4 = view2;
                        view3 = view;
                    }
    ...
    void layoutChunk(final Recycler recycler, final State state, final LayoutState layoutState, final LayoutChunkResult layoutChunkResult) {
        ...
        for (int i = 0; i < n3; ++i, mConsumed = n5) {
            ...
            if (this.mOrientation == 1) {
                this.measureChildWithDecorationsAndMargin(view, measureSpec, this.getMainDirSpec(layoutParams.height));
            }
            else {
                this.measureChildWithDecorationsAndMargin(view, this.getMainDirSpec(layoutParams.width), measureSpec);
            }
