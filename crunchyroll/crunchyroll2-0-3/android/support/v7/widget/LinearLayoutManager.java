...
public class LinearLayoutManager extends LayoutManager implements ViewDropHandler
{
    ...
    private View findFirstReferenceChild(final int n) {
        return this.findReferenceChild(0, ((RecyclerView.LayoutManager)this).getChildCount(), n);
    }
    ...
    private View findLastReferenceChild(final int n) {
        return this.findReferenceChild(((RecyclerView.LayoutManager)this).getChildCount() - 1, -1, n);
    }
    
    private View findReferenceChildClosestToEnd(final State state) {
        if (this.mShouldReverseLayout) {
            return this.findFirstReferenceChild(state.getItemCount());
        }
        return this.findLastReferenceChild(state.getItemCount());
    }
    
    private View findReferenceChildClosestToStart(final State state) {
        if (this.mShouldReverseLayout) {
            return this.findLastReferenceChild(state.getItemCount());
        }
        return this.findFirstReferenceChild(state.getItemCount());
        ...
        if (((RecyclerView.LayoutManager)this).getChildCount() != 0) {
            ...
            if (this.mLastStackFromEnd == this.mStackFromEnd) {
                ...
                if (anchorInfo.mLayoutFromEnd) {
                    view = this.findReferenceChildClosestToEnd(state);
                }
                else {
                    view = this.findReferenceChildClosestToStart(state);
                }
                if (view != null) {
    ...
    View findReferenceChild(int i, final int n, final int n2) {
        this.ensureLayoutState();
        View view = null;
        View view2 = null;
        final int startAfterPadding = this.mOrientationHelper.getStartAfterPadding();
    ...
    void onAnchorReady(final State state, final AnchorInfo anchorInfo) {
    }
    ...
    public View onFocusSearchFailed(View view, int convertFocusDirectionToLayoutDirection, final Recycler recycler, final State state) {
        ...
        if (((RecyclerView.LayoutManager)this).getChildCount() == 0) {
            ...
            if (convertFocusDirectionToLayoutDirection == -1) {
                view3 = this.findReferenceChildClosestToStart(state);
            }
            else {
                view3 = this.findReferenceChildClosestToEnd(state);
            }
            if (view3 == null) {
                return null;
            }
    ...
    static class LayoutState
    {
        ...
        private View nextViewFromScrapList() {
            for (int size = this.mScrapList.size(), i = 0; i < size; ++i) {
                final ViewHolder viewHolder = this.mScrapList.get(i);
                if (!viewHolder.isRemoved() && this.mCurrentPosition == viewHolder.getLayoutPosition()) {
                    this.assignPositionFromScrapList(viewHolder);
        ...
        public void assignPositionFromScrapList(ViewHolder nextViewHolderInLimitedList) {
            nextViewHolderInLimitedList = this.nextViewHolderInLimitedList(nextViewHolderInLimitedList);
            int layoutPosition;
            if (nextViewHolderInLimitedList == null) {
                layoutPosition = -1;
            }
            else {
                layoutPosition = nextViewHolderInLimitedList.getLayoutPosition();
            }
            this.mCurrentPosition = layoutPosition;
        }
        ...
        public ViewHolder nextViewHolderInLimitedList(final ViewHolder viewHolder) {
            ...
            while (true) {
                ...
                final ViewHolder viewHolder4 = this.mScrapList.get(n2);
                ...
                if (viewHolder4 != viewHolder) {
                    if (viewHolder4.isRemoved()) {
                        ...
                        final int n4 = (viewHolder4.getLayoutPosition() - this.mCurrentPosition) * this.mItemDirection;
                        viewHolder5 = viewHolder2;
                        n3 = n;
                        if (n4 >= 0) {
                            viewHolder5 = viewHolder2;
                            if (n4 < (n3 = n)) {
                                final ViewHolder viewHolder6 = viewHolder4;
                                n3 = n4;
