...
public class Vast1RootHandler implements VastRootStrategy
{
    ...
    public void startElement(final String s, final String s2, final String s3, final Attributes attributes) {
        if (Elements.Vast1Element.ROOT.getTag().equals(s2)) {
            this.playlist = new Playlist();
            return;
        }
        if (Elements.Vast1Element.AD.getTag().equals(s2)) {
            (this.delegate = new Vast1AdHandlerDelegate()).setLinearAd(new LinearAd());
