...
final class ExoPlayerImplInternal implements Handler$Callback
{
    ...
    public ExoPlayerImplInternal(final Handler eventHandler, final boolean playWhenReady, final int[] array, final int n, final int n2) {
        ...
        (this.internalPlaybackThread = new PriorityHandlerThread(this.getClass().getSimpleName() + ":Handler", -16)).start();
    ...
    private void doSomeWork() throws ExoPlaybackException {
        ...
        if (n != 0 && (this.durationUs == -1L || this.durationUs <= this.positionUs)) {
            this.setState(5);
            this.stopRenderers();
        }
        else if (this.state == 3 && n2 != 0) {
            this.setState(4);
            if (this.playWhenReady) {
                this.startRenderers();
            }
        }
        else if (this.state == 4 && n2 == 0) {
    ...
    private void ensureStopped(final TrackRenderer trackRenderer) throws ExoPlaybackException {
    ...
    private void incrementalPrepareInternal() throws ExoPlaybackException {
        ...
        int n6;
        long n7;
        for (int j = 0; j < this.renderers.length; ++j, n3 = n5, n4 = n6, durationUs = n7) {
            ...
            if (trackCount > 0) {
                ...
                if (n8 >= 0) {
                    ...
                    n7 = max;
                    if (n8 < array.length) {
                        trackRenderer2.enable(n8, this.positionUs, false);
    ...
    private void prepareInternal(final TrackRenderer[] renderers) throws ExoPlaybackException {
        this.resetInternal();
        this.renderers = renderers;
        Arrays.fill(this.trackFormats, null);
        for (int i = 0; i < renderers.length; ++i) {
            final MediaClock mediaClock = renderers[i].getMediaClock();
            if (mediaClock != null) {
    ...
    private boolean rendererReadyOrEnded(final TrackRenderer trackRenderer) {
        ...
        if (!trackRenderer.isEnded()) {
            ...
            if (this.state != 4) {
                ...
                if (n > 0L && bufferedPositionUs != -1L && bufferedPositionUs != -3L && bufferedPositionUs < this.positionUs + n) {
                    boolean b2 = b;
                    if (durationUs == -1L) {
                        return b2;
                    }
    ...
    private void seekToInternal(final long n) throws ExoPlaybackException {
        try {
    ...
    private <T> void sendMessageInternal(final int n, final Object o) throws ExoPlaybackException {
        try {
    ...
    private void setRendererSelectedTrackInternal(int n, final int n2) throws ExoPlaybackException {
        boolean b = true;
            ...
            if (this.state != 1 && this.state != 2) {
                ...
                if (state != 0 && state != -1 && trackRenderer.getTrackCount() != 0) {
                    ...
                    if (n != 0) {
                        ...
                        trackRenderer.enable(n2, this.positionUs, b);
    ...
    public void sendMessage(final ExoPlayer.ExoPlayerComponent exoPlayerComponent, final int n, final Object o) {
        ++this.customMessagesSent;
        this.handler.obtainMessage(9, n, 0, (Object)Pair.create((Object)exoPlayerComponent, o)).sendToTarget();
    }
